...
-(id)initWithX:(float)xVal y:(float)yVal
{
    if (self = [super init])
	{
		_x = xVal;
		_y = yVal;
		_segmentList = [[NSMutableArray alloc]initWithCapacity:2];
	}
	return self;
}
...
- (NSString *)description
{
	...
	[ms appendFormat:@"xy = [%g %g], number of segs - %ld\n",_x,_y,[_segmentList count]];
	for (unsigned i = 0;i < [_segmentList count];i++)
	{
		id s = _segmentList[i];
...
{
	return _segmentList[i];
}

- (NSMutableSet*)candidateSegmentsIntersect
{
	NSInteger ct = [_segmentList count];
	NSMutableSet *candidates = [NSMutableSet setWithCapacity:ct];
	for (int i = 0;i < ct;i++)
	{
		ISegElement *seg = [_segmentList objectAtIndex:i];
...
- (void)removeDuplicateSegments
   {
	for (unsigned i = 0;i < [_segmentList count];i++)
	   {
	    ISegElement *iSeg = [_segmentList objectAtIndex:i];
		for (NSUInteger j = [_segmentList count] - 1;j > i;j--)
		   {
			ISegElement *jSeg = [_segmentList objectAtIndex:j];
			if ([jSeg isGeometricallyTheSameAs:iSeg])
				[_segmentList removeObjectAtIndex:j];
		   }
	   }
   }
	...
	while (i < [_segmentList count])
	{
		...
		while (j < [[v segmentList]count])
		{
			...
			if ([jSeg isGeometricallyTheReverseOf:iSeg])
			{
				...
				[_segmentList removeObjectAtIndex:i];
				i--;
			}
			else
				j++;
