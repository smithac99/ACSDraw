		...
		for (int i = 0;i < rows;i++)
		   {
			...
			for (int j = 0;j < columns;j++)
				[a addObject:[[[NSTextStorage allocWithZone:[self zone]] init]autorelease]];
		...
		for (int i = 0;i < rows;i++)
		   {
			...
			for (int j = 0;j < columns;j++)
				[a addObject:[[[NSTextStorage allocWithZone:[self zone]] initWithAttributedString:[originalRow objectAtIndex:j]]autorelease]];
			[cellContents addObject:a];
		   }
		strokeType = strt;
		cellSizeFixed = csf;
...
- (id) initWithCoder:(NSCoder*)coder
   {
	...
	cellContents = [[coder decodeObjectForKey:@"ACSDMatrix_cellContents"]retain];
	cellStroke = [[coder decodeObjectForKey:@"ACSDMatrix_cellStroke"]retain];
		...
		for (int i = 0;i < rows;i++)
		   {
			NSMutableArray *a = [NSMutableArray arrayWithCapacity:columns];
			for (int j = 0;j < columns;j++)
				[a addObject:[[NSTextStorage allocWithZone:[self zone]] init]];
			[cellContents addObject:a];
		   }
	   }
