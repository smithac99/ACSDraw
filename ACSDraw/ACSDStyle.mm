...
+(ACSDStyle*)defaultStyle
   {
	ACSDStyle *a = [[[ACSDStyle alloc]initWithName:@"--No Style--"]autorelease];
	[a setNullStyle:YES];
	return a;
   }
...
+(NSMutableDictionary*)attributesFromTypingAttributes:(NSDictionary*)typing
   {
	ACSDStyle *st = [[[ACSDStyle alloc] initWithNoAttributes]autorelease];
	id o;
...
+(NSMutableArray*)initialStyles
   {
	...
	[m addObject:[[[ACSDStyle alloc]initWithName:@"Body text" font:[NSFont userFontOfSize:0]]autorelease]];
	NSFont *f = [[NSFontManager sharedFontManager]convertFont:[NSFont userFontOfSize:18]toHaveTrait:NSBoldFontMask];
	[m addObject:[[[ACSDStyle alloc]initWithName:@"Heading 1" font:f]autorelease]];
	f = [[NSFontManager sharedFontManager]convertFont:f toSize:16];
	[m addObject:[[[ACSDStyle alloc]initWithName:@"Heading 2" font:f]autorelease]];
	f = [[NSFontManager sharedFontManager]convertFont:f toSize:14];
	[m addObject:[[[ACSDStyle alloc]initWithName:@"Heading 3" font:f]autorelease]];
	[m addObject:[[[ACSDStyle alloc]initWithName:@"Code" font:[NSFont userFixedPitchFontOfSize:0]]autorelease]];
	return m;
   }
...
- (id) initWithCoder:(NSCoder*)coder
{
	...
	if (b)
		[self setBasedOn:b];
	attributes = [[coder decodeObjectForKey:StyleAttributesKey]retain];
...
+(NSParagraphStyle*)paragraphStyleForAttributes:(NSDictionary*)attrs existingParagraphStyle:(NSParagraphStyle*)existingPS
   {
	...
	if (existingPS)
		ps = [[existingPS mutableCopy]autorelease];
	else
