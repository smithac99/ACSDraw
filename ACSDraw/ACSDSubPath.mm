...
+(NSMutableArray*)subPathsFromBezierPath:(NSBezierPath*)path
{
    ...
    {
        ACSDSubPath *sub = [[ACSDSubPath alloc]init];
        BOOL closed;
...
-(id)init
{
    if (self = [super init])
    {
        pathElements = [NSMutableArray arrayWithCapacity:12];
        isClosed = NO;
    }
    return self;
}
...
{
    self = [super init];
    pathElements = [coder decodeObjectForKey:@"ACSDSubPath_pathElements"];
...
{
    return [pathElements copiedObjects];
}
...
- (id)copyWithZone:(NSZone *)zone 
{
    ...
    [obj setPathElements:[self copyOfPathElements]];
		...
		if ([pe deleteFollowingLine])
		   {
			...
			if ([sp isClosed])
			   ...
			   {
				ACSDSubPath *newSP = [[ACSDSubPath alloc]init];
...
-(KnobDescriptor)duplicateElement:(const KnobDescriptor&)kd
   {
	ACSDPathElement *el = [[pathElements objectAtIndex:kd.knob]copy];
	KnobDescriptor k = kd;
	k.knob++;
