...
- (id)tableView:(NSTableView *)aTableView objectValueForTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex
{
    ...
    {
        ACSDLayer *l = layerList[rowIndex];
              ...
              row:(int)rowIndex
{
    ...
    else if ([[aTableColumn identifier]isEqualTo:@"visible"])
    {
        if ([anObject isKindOfClass:[NSNumber class]])
            [layerList[rowIndex] setLayerVisible:[anObject boolValue]];
    }
    else if ([[aTableColumn identifier]isEqualTo:@"editable"])
    {
        if ([anObject isKindOfClass:[NSNumber class]])
            [layerList[rowIndex] setEditable:[anObject boolValue]];
    }
}
...
- (BOOL)tableView:(NSTableView *)tableView writeRowsWithIndexes:(NSIndexSet *)rowIndexes toPasteboard:(NSPasteboard*)pboard
{
    ...
    return [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:rowIndexes requiringSecureCoding:NO error:NULL] forType:ACSDrawLayerIntPasteboardType];
}
