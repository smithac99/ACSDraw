...
ShadowPanelController *_sharedShadowPanelController = nil;

@implementation ShadowPanelController

+ (id)sharedShadowPanelController
{
	if (!_sharedShadowPanelController)
		[PanelCoordinator sharedPanelCoordinator];
    return _sharedShadowPanelController;
}
...
{
	if (shadowList)
		[shadowList autorelease];
	if (f)
		shadowList = [f retain];
	else
		shadowList = nil;
	[[shadowTableView dataSource]setShadowList:f];
}
	...
	if (ct == 1)
	   {
		...
		if (shado == nil)
		{
			[shadowTableView selectRow:0 byExtendingSelection:NO];
			return;
		}
	   }
	if (g && shado)
	   {
		NSInteger pos = [shadowList indexOfObject:shado];
		if (pos == NSNotFound)
		...
		else
		{
			if (pos == [shadowTableView selectedRow])
				...
				[shadowTableView selectRow:pos byExtendingSelection:NO];
			return;
		}
	   }
}
...
-(void)duplicateShadowAtRow:(NSInteger)row
{
	...
	[shadowTableView reloadData];
	[shadowTableView selectRow:row + 1 byExtendingSelection:NO];
}
...
-(void)awakeFromNib
{
	_sharedShadowPanelController = self;
	[dragView setLeft:0.0];
	[dragView setRight:80.0];
	[super awakeFromNib];
	rowForContextualMenu = -1;
	displayRowForContextualMenu = -1;
	[[[shadowTableView tableColumns]objectAtIndex:0] setDataCell:[[[ShadowCell alloc]init] autorelease]];
	[[[shadowTableView tableColumns]objectAtIndex:4] setDataCell:[[[SelCell alloc]init]autorelease]];
