...
{
	[coder encodeConditionalObject:self.fromObject forKey:@"ACSDLink_fromObject"];
	[coder encodeConditionalObject:self.toObject forKey:@"ACSDLink_toObject"];
	...
	if (delegate && [delegate respondsToSelector:@selector(archiveType)] && [delegate archiveType] == ARCHIVE_PASTEBOARD)
		[coder encodeInt:[self.toObject objectKey] forKey:@"toObjectKey"];
}
